...
# --- Parameters ---
window_size = 5     # Change as needed
test_split = 0.3
MODEL_DIR = 'trained_model_v2'
OUTDIR = "data/datasets"
os.makedirs(MODEL_DIR, exist_ok=True)
os.makedirs(OUTDIR, exist_ok=True)
AGGRESSIVE_COUNT = 2000  # How many A and X samples after oversampling
...
def aggressive_oversample_windows(X_windowed, y_windowed, desired_count=2000, random_state=42):
    counts = Counter(y_windowed)
    sampling_strategy = {k: max(counts[k], desired_count) for k in counts}
    sampling_strategy[0] = desired_count  # A
    sampling_strategy[4] = desired_count  # X
    ...
    print("Class balance after aggressive oversampling:", Counter(y_resampled))
    return X_resampled, y_resampled

print("Class balance before oversampling:", Counter(y_train_w))
X_train_bal, y_train_bal = aggressive_oversample_windows(X_train_w, y_train_w, desired_count=AGGRESSIVE_COUNT)
...
# --- Save Model & History ---
model_path = os.path.join(MODEL_DIR, f'solarflare_tcn_aggressive_oversample_ws{window_size}.h5')
model.save(model_path)
with open(os.path.join(MODEL_DIR, f'train_history_aggressive_oversample_ws{window_size}.pkl'), 'wb') as f:
...
}).to_csv(os.path.join(OUTDIR, f'comparison_test_aggressive_oversample_ws{window_size}.csv'), index=False)
